...
SEQUENCE_LENGTH = 11  # Use 10 consecutive depth readings
...
class LSTMClassifier(nn.Module):
    def __init__(
        ...
        # BiLSTM outputs have 2x hidden_size (forward + backward)
    ...
    def forward(self, x):
        ...
        # For BiLSTM, concatenate forward and backward hidden states from last layer
        # hidden shape: (num_layers * 2, batch_size, hidden_size)
        forward_hidden = hidden[-2, :, :]
        backward_hidden = hidden[-1, :, :]
        combined_hidden = torch.cat((forward_hidden, backward_hidden), dim=1)
        out = self.fc(combined_hidden)  # (batch_size, num_classes)
        return out

class BILSTM(BaseModel):
    def __init__(self, sequence_length=11, hidden_size=128, num_layers=1, dropout=0.3):
        super().__init__()
    ...
    def create_sequences(self, data, targets, sequence_length=11):
        ...
        Each sequence = sequence_length consecutive readings centered around the target.
        Target = the rock type at the center of the sequence.
        """
        sequences = []
        target_labels = []
        
        half = sequence_length // 2
        # Start from half to avoid going negative, end before going out of bounds
        for idx in range(half, len(data) - (sequence_length - half)):
            # Center the window around idx
            sequences.append(data[idx - half:idx + (sequence_length - half)])
    ...
    def _train(self, data: ModelDataset) -> None:
    ...
    def train(self, data: ModelDataset) -> None:
        ...
        study = optuna.create_study(
            direction="maximize",
            storage=storage,
            study_name="bilstm_default_effective_sampler",
            load_if_exists=True,
        )
