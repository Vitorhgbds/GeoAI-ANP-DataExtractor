        ...
        try:
            with dlis.load(f"{file_name}") as dlis_file:
                ...
                for logical_file in dlis_file:
                    ...
                    channels: list[Channel] = [c for c in file.channels if c.name.upper() in valid_channels]
                    ...
                    for i in tqdm(
                        range(0, len(index_curves) - 1), desc=f"Processing file {file}", leave=False, miniters=1
                    ):
    ...
    def process(self, log: LogDownloadDTO) -> list[dict]:
        ...
        try:
            with dlis.load(f"{file_name}") as dlis_file:
                ...
                data_points = [
                    LogChannelsDTO(well=log.well, channel=channel_name, total_data=total_data)
